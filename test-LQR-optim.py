...
for t in range(T):
    ...
    A = t*np.ones((n,n))/10. + n_ones @ Arow / 1000. + Arow.T @ n_ones.T / 100.
    # ppm.ppm(A,f"A[{t}]",sigfigs=3)

    m_ones = np.ones((m,1))
    Brow  = np.array(range(1,m+1)).reshape((1,m))
    B = -(t*np.ones((n,m))/10. + n_ones @ Brow / 1000. + Arow.T @ m_ones.T / 100.)
...
for t in range(T+1):
    ...
    C0 +=  10*np.eye(n+m)
    C0 += np.ones((m+n,1)) @ np.array(range(1,m+n+1)).reshape((1,m+n)) / 200.
    C0 += np.array(range(1,m+n+1)).reshape((m+n,1)) @ np.ones((1,m+n)) / 20.
    C0  = (C0 + C0.T)/2.0
...
if True:
    ...
    if Mrows>32 or Mcols > 325:
        print(f"KKT matrix has shape {Mrows} x {Mcols}. That's too big. Printing suppressed.")
    else:
    ...
    wrows,wcols = w.shape
    if brows>32:
        print(f"KKT solution vector w has shape {wrows} x {wcols}. Printing suppressed.")
    else:
    ...
    for r in range(0,T):
        ...
        KKTx[:,[0],r+1] = w[(m+n+r*(m+2*n)):(m+n+r*(m+2*n)+n),:][:]
